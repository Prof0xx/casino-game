...
function Sign(props) {
    ...
    function signSubmit(data){
        ...
        // Check if wallet is connected before allowing authentication
        if (!walletConnected) {
        ...
        if(!checkPayload(data.payload)){
            // Add wallet information to the payload
            ...
            setLoaded(false)
            socket.emit(data.emit, enhancedPayload)
        }
    }
    ...
    function handleWalletConnected(address, type) {
        ...
        setWalletType(type)
        setWalletConnected(!!address) // Convert to boolean
    }
